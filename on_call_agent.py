...
TWILIO_WHATSAPP_NUMBER = os.getenv("TWILIO_WHATSAPP_NUMBER", "whatsapp:x")
USER_WHATSAPP_NUMBER = os.getenv("USER_WHATSAPP_NUMBER", "whatsapp: x")
...
async def prom_instant_query(expr: str) -> float:
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            r = await client.get(f"{PROM_URL}/api/v1/query", params=q, headers=headers)
            r.raise_for_status()
            js = r.json()

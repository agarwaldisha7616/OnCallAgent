...
# service: which microservice handles the request
...
# Removed rigorous similualtion helpers to keep endpoint simple

 # ---- FastAPI app ----
app = FastAPI(title= f"InventoryService[{SERVICE}]")

# middleware is function that works for every request
@app.middleware("http")
...
# Exposes Prometheus metrics for monitoring the service.
@app.get("/metrics")
def metrics():
...
# Returns recent log entries from the shared log file.
@app.get("/logs/tail")
def tail_logs(seconds: int = 15, limit: int = 500):
    """Return the recent log from the common log within the last N seconds.
    ...
    filtered = []
    for entry in parsed:
        tsobj = entry.get("_tsobj")
        if tsobj is None or tsobj >= cutoff:
            filtered.append({k: v for k, v in entry.items() if k != "_tsobj"})
...
# Simulates a null pointer exception to test error handling.
